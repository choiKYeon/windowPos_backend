...
//        redis에 저장
        doNothing().when(redisService).setValues(accessToken, username);
        doNothing().when(redisService).setValues(refreshToken, username);
        ...
        // 로그인 요청
        ResponseEntity<RsData> loginResponse = restTemplate.exchange(createURLWithPort("/api/v1/member/login"), HttpMethod.POST, entity, RsData.class);
        ...
        verify(redisService, times(1)).getValue(accessToken);
        verify(redisService, times(1)).getValue(refreshToken);
        ...
        logoutHeaders.set("Authorization", accessToken);
        HttpEntity<String> logoutEntity = new HttpEntity<>(logoutHeaders);

        ResponseEntity<RsData> logoutResponse = restTemplate.exchange(createURLWithPort("/api/v1/member/logout"), HttpMethod.POST, logoutEntity, RsData.class);
